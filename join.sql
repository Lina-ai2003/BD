					        ...
					        AND a.cnum = d.cnum
						        AND a.snum = d.snum
							        AND d.snum = b.snum
									    ...
									    /*использоввание join ддля объединения результатов выборки из трех таблиц*/
SELECT 
*
FROM
    sales a
    ...
    получаем список покупателей из таблицы customers и null значенияя там, где покупателям не назначен продавец*/
...
FROM
    sales a
	    ...
	    join customers c using (cnum);
	      ...
	      /*с таблицей , полученой в результате соединений, можно использовать условия в предложении where*/ 
        SELECT 
	*
	FROM
	    sales a
